...
variable "private_dns_zone_id_cognitive_service_openai" {
  description = "Specifies the resource ID of the private DNS zone for Azure Open AI endpoints."
  type        = string
  sensitive   = false
  validation {
    condition     = var.private_dns_zone_id_cognitive_service_openai == "" || (length(split("/", var.private_dns_zone_id_cognitive_service_openai)) == 9 && endswith(var.private_dns_zone_id_cognitive_service_openai, "privatelink.openai.azure.com"))
    error_message = "Please specify a valid resource ID for the private DNS Zone."
  }
}
